...
def showEdge(node_info, colour, width):
    ...
    while True:
        try:
            # Every alternate node_info entry is the row/col
            neighbour_col=int(node_info[i])
            neighbour_row=int(node_info[i+1])
            pygame.draw.line(screen, colour, (col, row), (neighbour_col, neighbour_row), width)
            i+=2;
        except:
            break
...
with open('../src/graph.csv', 'r') as csv_file:
    reader = csv.reader(csv_file, delimiter = ',')
